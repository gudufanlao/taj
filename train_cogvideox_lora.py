...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        #video_frames = self._load_and_preprocess_video(self.instance_video_paths[index])
        return {
            "instance_prompt": self.id_token + self.instance_prompts[index],
            "instance_video": self.instance_videos[index],
        }
    # def _load_and_preprocess_video(self, filename):
    #     try:
    #         import decord
    #     except ImportError:
    #         raise ImportError("The `decord` package is required for loading the video dataset. Install with `pip install decord`")
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        #torch.cuda.empty_cache()  # 新增：手动释放 GPU 显存
